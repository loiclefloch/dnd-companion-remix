...
import { getLevellingDataForClassesAndLevel, getLevelExperienceStage, getNextLevelExperienceStage } from "../modules/levelling"
import { getProficiencyBonus } from "../modules/levelling"
import { getSpellsForCharacterSubclass } from "../modules/character/subclass"
import { valueToModifier, valueToModifierLabel, modifierToModifierLabel } from "../modules/stats"
import { filterDuplicates } from '../modules/utils/array'
import applyFeaturesOnCharacter from "../modules/character/applyFeaturesOnCharacter"
import applyTraitsOnCharacter from "../modules/character/applyTraitsOnCharacter"
import applyFeatsOnCharacter from "../modules/character/applyFeatsOnCharacter"
...
import type { CharacterDetailsApiObject, PersonalityTraitsApiObject, IdealsApiObject } from '../apiobjects/character.apiobject';
import type { IdealsDto, PersonalityTraitsDto, CharacterDetailsDto } from '~/dtos/character.dto'

export function transformIdeals(ideals: IdealsApiObject): IdealsDto {
  return ideals
}
...
export function formatCharacter(
  ...
  if (character.subclass) {
    character.subclass = {
      ...formatSubclass(
        subclasses.find((c) => c.index === character.subclass.index)
      ),
    };
  }
  const clss = character.classes[0];

  const levellingData = getLevellingDataForClassesAndLevel(
    character.classes,
    character.level
  );
  ...
  character.background = backgrounds.find(
    (b) => b.index === character.background
  );

  character.alignment = alignments.find((a) => a.index === character.alignment);

  character.spellcasting = {
    ...(levellingData.spellcasting || {}),
    ...(clss.spellcasting || {}),
  };
  character.spellcastingAbility = clss.spellcasting?.spellcastingAbility?.name;
  character.spellcastingAbilityValue = valueToModifier(
    character.stats[character.spellcastingAbility] || 0
  );
  character.spellcastingAbilityValueLabel = modifierToModifierLabel(
    character.spellcastingAbilityValue
  );
  ...
  character.proficiencyBonus = getProficiencyBonus(
    character.classes[0].index,
    character.level
  );
  ...
  character.spellSaveDC =
    8 + character.proficiencyBonus + character.spellcastingAbilityValue;
  ...
  character.spellAttackBonus =
    character.proficiencyBonus + character.spellcastingAbilityValue;

  character.meleeAttackRollModifier = valueToModifier(character.stats.STR);
  character.meleeAttackRollModifierLabel = valueToModifierLabel(
    character.stats.STR
  );

  character.rangedAttackRollModifier = valueToModifier(character.stats.DEX);
  character.rangedAttackRollModifierLabel = valueToModifierLabel(
    character.stats.DEX
  );
  ...
  character.skills = skills.map((skillData) => {
    ...
    const modifier =
      valueToModifier(value) + (isProeficient ? character.proficiencyBonus : 0);
    return {
  ...
  const isProefficientInPerception = character.skills.find(
    (s) => s.index === "perception"
  ).isProeficient;
  character.passivePerception =
    10 +
    valueToModifier(character.stats.WIS) +
    (isProefficientInPerception ? character.proficiencyBonus : 0);

  const isProefficientInInvestigation = character.skills.find(
    (s) => s.index === "investigation"
  ).isProeficient;
  character.passiveInvestigation =
    10 +
    valueToModifier(character.stats.INT) +
    (isProefficientInInvestigation ? character.proficiencyBonus : 0);
  ...
  function buildSavingThrow(ability) {
    const isProeficient = character.classes[0].savingThrows.some(
      (savingThrow) => savingThrow.name === ability
    );
    const value =
      character.stats[ability] +
      (isProeficient ? character.proficiencyBonus : 0);
    return {
      value,
  ...
  character.actionsEquipment = [
    formatEquipmentItem(
      equipmentList.find((i) => i.index === "unarmed-strike")
    ),
    ...character.equipment.filter(
      (item) => item.equipmentCategory?.index === "weapon"
    ),
  ]
    .map(camelize)
  ...
  // TODO:
  const armorEquipped = character.equipment.find(
    (item) => item.isArmor && item.isEquipped
  );
  const shieldEquipped = character.equipment.find(
    (item) => item.isShield && item.isEquipped
  );
  ...
  function isProeficientForArmor() {
    ...
    const allArmor = character.proficiencies.some(
      (p) => p.index === "all-armor"
    );
    if (allArmor) {
      return true;
  ...
  // TODO: handle
  character.speedReducedDueToArmorStrength = armorEquipped
    ? armorEquipped.strMinimum > character.stats.STR
    : false;
  ...
  map(character.abilityDisadvantage, (_, ability) => {
    const skillsForStat = skills.filter(
      (s) => s.ability_score.index === ability
    );
  ...
  character.currentSpeed =
    character.baseSpeed + (character.speedReducedDueToArmorStrength ? -10 : 0);

  // add subclass features
  character.subclassFeatures = getCharacterSubclassFeatures(character);
  character.features = filterDuplicates(
    [...character.features, ...character.subclassFeatures],
    (f) => f.index
  );
  ...
  character.ac = {
    ...
    shield: shieldAc,
    total:
      (hasArmorEquipped ? armorAc : character.naturalAc) +
      (hasShieldEquipped ? shieldAc : 0),
  };
